...
window.promptTransferAdmin = promptTransferAdmin;
window.executeTransferAdmin = executeTransferAdmin;
...
function checkCanInvite() {
    if (!activeGroupData) return false;
    const invitePerm = activeGroupData.invitePermission || 'admin_and_officers';
    if (invitePerm === 'all_members') return true;
    if (invitePerm === 'admin_and_officers') return activeGroupRole === 'admin' || activeGroupRole === 'officer';
    return activeGroupRole === 'admin';
}
...
async function viewUserProfile(uid) {
    ...
    let inviteBtnHtml = '';
    const canInvite = checkCanInvite();
    if (activeGroupId && canInvite && uid !== currentUser.uid) {
    ...
    openModal(`
        ...
        <div style="text-align:center; margin-bottom:16px;">
            ...
            <p style="font-size:13px; color:var(--text-muted); margin-top:6px;">${u.bio || "Hey there! I am using TIME & RULE."}</p>
        </div>
...
async function inviteUserToGroup(targetUid, targetName) {
    if (!currentUser || !checkCanInvite()) return;
...
function openCreateGroupModal() {
    ...
    document.getElementById('create-group-form').addEventListener('submit', async (e) => {
        ...
        const groupData = {
            ...
            approvalBy: 'admin_only',
            invitePermission: 'admin_and_officers',
            officerInviteRequiresApproval: false,
            createdAt: serverTimestamp()
        };
...
async function joinGroup(groupId) {
    ...
    const grpData = grpSnap.exists() ? grpSnap.val() : {};

    if (grpData.requireApproval) {
        const reqRef = push(ref(db, `groupRequests/${groupId}`));
        await set(reqRef, {
            type: 'join_request',
            uid: currentUser.uid,
        ...
        hideLoader();
        closeModal();
        showToast("Join request sent to admin/officers for approval.", "success");
        return;
    }
...
function renderGroupDashboardContainer() {
    ...
    container.innerHTML = `
                ...
                ${isAdmin ? `
                    <div class="group-options-wrapper">
                        ...
                        <div class="group-options-dropdown">
                            ...
                            <div class="group-dropdown-item" onclick="switchGroupSubTab('settings')">⚙️ Group Settings</div>
        ...
        <div class="group-nav-tabs">
            ...
            ${isAdmin ? `<button type="button" class="group-sub-tab" data-sub="settings" onclick="switchGroupSubTab('settings')">Settings</button>` : ''}
        </div>
...
function renderGroupSettings(container) {
    if (!currentUser || activeGroupRole !== 'admin') {
        container.innerHTML = '<div class="empty-state">Only admins can access group settings.</div>';
        return;
    }

    container.innerHTML = `
        <div class="glass-card" style="max-width: 600px; padding: 24px; margin: 0 auto;">
            <h3 style="margin-bottom: 16px;">⚙️ Group Settings & Permissions</h3>
            
            <div class="input-group" style="margin-bottom: 20px;">
                <label style="font-weight: 700; font-size: 14px;">Who Can Invite New Members?</label>
                <select id="setting-invite-permission" style="margin-top: 6px;">
                    <option value="admin_only" ${activeGroupData.invitePermission === 'admin_only' ? 'selected' : ''}>Only Admin</option>
                    <option value="admin_and_officers" ${!activeGroupData.invitePermission || activeGroupData.invitePermission === 'admin_and_officers' ? 'selected' : ''}>Admin & Officer</option>
                    <option value="all_members" ${activeGroupData.invitePermission === 'all_members' ? 'selected' : ''}>All Members</option>
                </select>
            </div>

            <div class="input-group" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                <div>
                    <label style="font-weight: 700; font-size: 14px;">Require Join Approval</label>
                    <p style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">New join requests require manual approval.</p>
                </div>
                <input type="checkbox" id="setting-require-approval" ${activeGroupData.requireApproval ? 'checked' : ''} style="width: 20px; height: 20px; accent-color: var(--accent-cyan);">
            </div>

            <div class="input-group" style="margin-bottom: 20px;">
                <label style="font-weight: 700; font-size: 14px;">Who Can Approve Joins / Invites?</label>
                <select id="setting-approval-by" style="margin-top: 6px;">
                    <option value="admin_only" ${activeGroupData.approvalBy === 'admin_only' ? 'selected' : ''}>Admin Only</option>
                    <option value="admin_and_officers" ${activeGroupData.approvalBy === 'admin_and_officers' ? 'selected' : ''}>Admin & Officers</option>
                </select>
            </div>

            <div class="input-group" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                <div>
                    <label style="font-weight: 700; font-size: 14px;">Officer Invite Requires Approval</label>
                    <p style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">If enabled, members invited by officers must be approved by admin/officers.</p>
                </div>
                <input type="checkbox" id="setting-officer-invite-approval" ${activeGroupData.officerInviteRequiresApproval ? 'checked' : ''} style="width: 20px; height: 20px; accent-color: var(--accent-cyan);">
            </div>

            <button type="button" class="futuristic-btn primary full-width" onclick="saveGroupSettings()">Save Settings</button>
        </div>
    `;
}
...
async function saveGroupSettings() {
    ...
    const invitePermission = document.getElementById('setting-invite-permission').value;
    const requireApproval = document.getElementById('setting-require-approval').checked;
    const approvalBy = document.getElementById('setting-approval-by').value;
    const officerInviteRequiresApproval = document.getElementById('setting-officer-invite-approval').checked;

    showLoader();
    try {
        await update(ref(db, `groups/${activeGroupId}`), { invitePermission, requireApproval, approvalBy, officerInviteRequiresApproval });
        activeGroupData.invitePermission = invitePermission;
        activeGroupData.requireApproval = requireApproval;
        activeGroupData.approvalBy = approvalBy;
        activeGroupData.officerInviteRequiresApproval = officerInviteRequiresApproval;
        hideLoader();
...
function switchGroupSubTab(subtab) {
    ...
    if (subtab === 'settings') renderGroupSettings(contentArea);
}
            ...
            for (const gId of Object.keys(groupsData)) {
                ...
                if (reqSnap.exists()) {
                    ...
                    for (const reqId of Object.keys(reqs)) {
                        ...
                        if (req.type === 'group_invite' && req.targetUid === currentUser.uid && req.status === 'pending') {
                            html += `
                                <div class="glass-card" style="border: 1px solid var(--accent-cyan);">
                                    ...
                                    <p style="font-size: 13px; color: var(--text-muted); margin: 6px 0;">Invited by: ${req.inviterName || 'Admin'} (${(req.inviterRole || 'admin').toUpperCase()})</p>
                                    <div style="display: flex; gap: 10px; margin-top: 12px;">
                                        <button type="button" class="futuristic-btn small primary" onclick="respondGroupInvite('${gId}', '${reqId}', true)">Accept (Join)</button>
                                        <button type="button" class="futuristic-btn small danger" onclick="respondGroupInvite('${gId}', '${reqId}', false)">Reject</button>
                                    </div>
                                </div>
                            `;
                        }

                        if (req.type === 'join_request' && req.status === 'pending') {
                            const userMembership = membershipsSnap.exists() ? membershipsSnap.val()[gId] : null;
                            const isGroupAdminOrOfficer = userMembership === 'admin' || userMembership === 'officer';
                            const canApprove = gChild.approvalBy === 'admin_and_officers' ? isGroupAdminOrOfficer : (userMembership === 'admin' || gChild.adminUid === currentUser.uid);

                            if (canApprove) {
                                html += `
                                    <div class="glass-card" style="border: 1px solid var(--accent-purple);">
                                        <span class="badge" style="background:var(--accent-purple); color:#000;">JOIN REQUEST</span>
                                        <h4 style="margin-top: 8px;">User: ${req.displayName} (@${req.username})</h4>
                                        <p style="font-size: 13px; color: var(--text-muted); margin: 6px 0;">Wants to join group: ${gChild.name}</p>
                                        <div style="display: flex; gap: 10px; margin-top: 12px;">
                                            <button type="button" class="futuristic-btn small primary" onclick="respondJoinRequest('${gId}', '${reqId}', '${req.uid}', true)">Approve</button>
                                            <button type="button" class="futuristic-btn small danger" onclick="respondJoinRequest('${gId}', '${reqId}', '${req.uid}', false)">Reject</button>
                                        </div>
                                    </div>
                                `;
                            }
                        ...
                        if (req.status === 'pending' && req.type !== 'group_invite' && req.type !== 'join_request') {
                            if (membershipsSnap.exists() && membershipsSnap.val()[gId]) {
...
async function respondJoinRequest(groupId, requestId, targetUid, isApproved) {
    showLoader();
    try {
        const reqRef = ref(db, `groupRequests/${groupId}/${requestId}`);
        if (isApproved) {
            await set(ref(db, `memberships/${targetUid}/${groupId}`), 'member');
            await set(ref(db, `groupMembers/${groupId}/${targetUid}`), {
                uid: targetUid,
                role: 'member',
                joinedAt: serverTimestamp()
            });
        ...
        console.error("Error in respondJoinRequest:", err);
        showToast("Error processing request: " + err.message, "danger");
    } finally {
        hideLoader();
        loadRequests();
    }
...
async function respondGroupInvite(groupId, requestId, isAccepted) {
    ...
    try {
        ...
        if (isAccepted) {
            ...
            const isOfficerInvite = req.inviterRole === 'officer';
            const officerInviteRequiresApproval = grpData.officerInviteRequiresApproval === true;

            if (isOfficerInvite && officerInviteRequiresApproval) {
                await update(reqRef, { 
                    type: 'join_request', 
                    status: 'pending', 
                    uid: currentUser.uid, 
                ...
                hideLoader();
                closeModal();
                showToast("Your join request has been sent for admin/officer approval.", "success");
                loadRequests();
                return;
            }
...
async function renderGroupMembers(container) {
    ...
    let html = `
        <div class="section-header-row" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
            <h3>Group Members</h3>
            ${canInvite ? '<button type="button" class="futuristic-btn small primary" onclick="openAdminInviteModal()">+ Invite Member</button>' : ''}
        </div>
    ...
    if (membersSnap.exists()) {
        ...
        membersList.sort((a, b) => {
            ...
            const roleWeight = { admin: 1, officer: 2, member: 3 };
            return (roleWeight[a.role] || 3) - (roleWeight[b.role] || 3);
        });

        for (const child of membersList) {
            const uSnap = await get(ref(db, `users/${child.uid}`));
            const uInfo = uSnap.exists() ? uSnap.val() : {};
            
            let dotMenuHtml = '';
            const isSelf = child.uid === currentUser.uid;
            const isAdminViewer = activeGroupRole === 'admin';
            const isOfficerReviewingMember = activeGroupRole === 'officer' && child.role === 'member';

            if (!isSelf && (isAdminViewer || isOfficerReviewingMember)) {
                dotMenuHtml = `
                    <div class="group-options-wrapper" style="position:relative; display:inline-block;" onclick="event.stopPropagation();">
                        <button type="button" class="group-three-dots-btn" onclick="toggleGroupOptionsMenu(event)">⋮</button>
                        <div class="group-options-dropdown">
                            ${isAdminViewer ? `
                                <div class="group-dropdown-item" onclick="promptTransferAdmin('${child.uid}', '${uInfo.displayName || 'Member'}')">👑 Transfer Admin</div>
                                <div class="group-dropdown-item" onclick="toggleOfficer('${child.uid}', '${child.role}')">${child.role === 'officer' ? '⬇️ Remove Officer' : '⭐ Make Officer'}</div>
                            ` : ''}
                            <div class="group-dropdown-item danger" onclick="promptRemoveMember('${child.uid}', '${uInfo.displayName || 'Member'}')">🚪 Remove Member</div>
                        </div>
                    </div>
                `;
            }
            ...
            html += `
                <div class="glass-card" onclick="viewUserProfile('${child.uid}')" style="display: flex; justify-content: space-between; align-items: center; padding: 12px 20px; cursor: pointer;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <div>
                            <strong>${uInfo.displayName || 'Member'} ${isSelf ? '(You)' : ''}</strong>
...
function promptTransferAdmin(targetUid, targetName) {
    if (!currentUser || activeGroupRole !== 'admin') return;
    openModal(`
        <div class="modal-header">
        ...
        <p style="font-size:13px; color:var(--text-muted); margin-bottom:16px;">Are you sure you want to transfer Admin rights to <strong>${targetName}</strong>? You will become a regular member/officer.</p>
        <button type="button" class="futuristic-btn danger full-width" onclick="executeTransferAdmin('${targetUid}')">Confirm Transfer</button>
    `);
}

async function executeTransferAdmin(targetUid) {
    showLoader();
    try {
        ...
        hideLoader();
        closeModal();
        showToast("Admin ownership transferred successfully!", "success");
...
async function toggleOfficer(targetUid, currentRole) {
    ...
    if (currentRole === 'officer') {
        await update(ref(db, `groupMembers/${activeGroupId}/${targetUid}`), { role: 'member' });
        hideLoader();
        closeModal();
        showToast("Officer status removed.", "success");
        renderGroupMembers(document.getElementById('group-subtab-content'));
    } else {
        const memSnap = await get(ref(db, `groupMembers/${activeGroupId}`));
        let officerCount = 0;
        if (memSnap.exists()) {
            memSnap.forEach(child => {
                if (child.val().role === 'officer') officerCount++;
            });
        }
        if (officerCount >= 3) {
            hideLoader();
            showToast("Maximum limit of 3 officers reached.", "warning");
            return;
        }

        await update(ref(db, `groupMembers/${activeGroupId}/${targetUid}`), { role: 'officer' });
        hideLoader();
        closeModal();
        showToast("Promoted to Officer successfully!", "success");
        renderGroupMembers(document.getElementById('group-subtab-content'));
    }
}

function promptRemoveMember(targetUid, targetName) {
    openModal(`
        <div class="modal-header">
            <h3>Remove Member</h3>
            <button type="button" class="close-modal-btn" onclick="closeModal()">×</button>
        </div>
        <p style="font-size:13px; color:var(--text-muted); margin-bottom:16px;">Are you sure you want to remove <strong>${targetName}</strong> from this group?</p>
        <button type="button" class="futuristic-btn danger full-width" onclick="executeRemoveMember('${targetUid}')">Confirm Removal</button>
    `);
}

async function executeRemoveMember(targetUid) {
    showLoader();
    try {
        await remove(ref(db, `groupMembers/${activeGroupId}/${targetUid}`));
        await remove(ref(db, `memberships/${targetUid}/${activeGroupId}`));
        hideLoader();
        closeModal();
